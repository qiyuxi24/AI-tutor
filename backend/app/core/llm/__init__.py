...
- embed.embed_texts                              **检索侧**嵌入唯一出口（kb/rag 向量化共用；
                                                  语义去重侧走 kb/embedder.py，共用同名常量）
...
- json_extract.extract_json                     从 LLM 响应提取 JSON（三策略，全库唯一）
...
from app.core.llm.embed import EMBED_BATCH_SIZE, EMBEDDING_MODEL, MAX_EMBED_CHARS, embed_texts
