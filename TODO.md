...
> 校内截止：2026-10-15 ｜ 今日：2026-09-05（W1 末，距截止 6 周）
>
> **⚠️ 清单状态已按代码实测校准（2026-09-05 全库核查），非按文档印象勾选。**

---

## 📋 工程化就绪度评估结论（2026-09-05，详见代码核查）

**结论：代码底子已够"工程化"说事，缺的是 文档可信度 + 演示数据 + 一条可用的真实 LLM key。**

| 维度 | 等级 | 关键证据 |
|------|------|---------|
| 架构/分层 | A | rag_pipeline / kb / hybrid_search / quiz / collector 全部去耦合，注册表+策略模式 |
| 自动化测试 | A | 全库 **211 passed / 11s 零网络**（单元 90+ + 集成 11 + collector 系列） |
...
- [ ] **真实 LLM 链路验证**（需 DASHSCOPE 额度恢复后逐项跑）
  - [ ] quiz 简答 LLM 判分端到端联调
...
- [ ] **图谱重复节点处理**：`create_node_from_ai` 对已存在 ID 应"追加内容/改更新"，而非直接 `add_node` 报重复
- [ ] **演示数据 + 脚本**：填充 1 个完整学科图谱（15-20 节点 + prerequisite 边）；写 2-3 条预演对话路径
- [ ] **工程卫生**：清理 `.bak`/`_tmp_*.py` 残留；git commit 规范（类型前缀）
...
- [ ] **聊天接口限流**（现有限流只覆盖 auth）
- [ ] **结构化日志**（文件输出/轮转，可观测性）
- [ ] **.github CI**：push 自动 `pytest`（后端已有 211 用例，接入成本极低）
- [ ] **节点掌握度手动调整**：NodeDetail 加滑块（后端 PUT /node/{id}/mastery 已有）
- [ ] **图谱知识一键导出**：`/knowledge/export?subject=` 合并 Markdown + 前端下载按钮
- [ ] **Prompt 笔记优化**：教学后主动 `update_node_content`/`add_knowledge_node` 记笔记；text→Markdown 分屏编辑

## P2 — 功能路线图（比赛可选项 / 有真实 key 后）

- [ ] **RAG 精排（Cross-Encoder）**：hybrid_search 后接 rerank，量化收益（W2 主线）
...
- [x] 两阶段流式对话（SSE 文本 + 后台 function calling）+ 四种教学模式
...
## 进度速览（2026-09-05）
- 代码层工程化 ~70%（架构✓ 测试✓ 部署✓ / 文档✗ CI✗ 运维✗）
- 比赛落地待办：P0 约 2 周 → 材料期 10/5 起（作品文档 + 演示视频 + 申报）
- W2（9/7-9/13）主线：RAG 精排 —— **前提：恢复真实 LLM key**
