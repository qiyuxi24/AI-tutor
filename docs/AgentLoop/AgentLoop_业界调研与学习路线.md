...
> 创建：2026-09-06
> 定位：为「对话 Agent 循环（Agent Loop）重构」提供**业界成熟做法的调研整理 + 一手资料清单**，学习时对照代码。结论与落地取舍仍以 `docs/AgentLoop/AgentLoop_重构设计讨论.md`（决策载体）为准，本文档给出"业界参照物"。
> 用法：按「二、逐家拆解」对照源码/文档回看；实现 Batch1 前先读「三、对照表」与「四、落地增量建议」。
> 前置阅读：`docs/AgentLoop/AgentLoop_重构设计讨论.md`（v0.1，现状诊断 + 路线 A/B/C + 里程碑）
...
2. 我们 `docs/AgentLoop/AgentLoop_重构设计讨论.md` §2 诊断的六个事实，业界各自怎么处理 —— 差距在哪。
...
- 本文档只提供"业界参照 + 学习清单"，不替代 `docs/AgentLoop/AgentLoop_重构设计讨论.md` 的决策职能。
